...
// Hash: f7d4b7227dc3b524b3fb633095154ac7b2b48cba8b4007551f73e34360ab96c3
// Version: 0.1.3
package deneb
...
func (b *BlindedBlobsBundle) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := len(b.Commitments); size > 4 {
		err = ssz.ErrListTooBigFn("BlindedBlobsBundle.Commitments", size, 4)
		return
	}
	...
	if size := len(b.Proofs); size > 4 {
		err = ssz.ErrListTooBigFn("BlindedBlobsBundle.Proofs", size, 4)
		return
	}
	...
	if size := len(b.BlobRoots); size > 4 {
		err = ssz.ErrListTooBigFn("BlindedBlobsBundle.BlobRoots", size, 4)
		return
	}
