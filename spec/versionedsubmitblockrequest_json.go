...
type fuluVersionedSubmitBlockRequestJSON struct {
	Data *fulu.SubmitBlockRequest `json:"data"`
}
...
func (v *VersionedSubmitBlockRequest) MarshalJSON() ([]byte, error) {
	...
	case spec.DataVersionElectra:
		...
		data := &electraVersionedSubmitBlockRequestJSON{
			Data: v.Electra,
		}
		payload := struct {
			*versionJSON
			*electraVersionedSubmitBlockRequestJSON
		}{version, data}
	...
	case spec.DataVersionFulu:
		...
		payload := struct {
			*versionJSON
			*fuluVersionedSubmitBlockRequestJSON
		}{version, data}
...
func (v *VersionedSubmitBlockRequest) UnmarshalJSON(input []byte) error {
	...
	case spec.DataVersionDeneb:
		...
		v.Deneb = data.Data
	case spec.DataVersionElectra:
		var data electraVersionedSubmitBlockRequestJSON
		if err := json.Unmarshal(input, &data); err != nil {
			return errors.Wrap(err, "invalid JSON")
		}
		...
		v.Electra = data.Data
	case spec.DataVersionFulu:
		var data fuluVersionedSubmitBlockRequestJSON
		if err := json.Unmarshal(input, &data); err != nil {
			return errors.Wrap(err, "invalid JSON")
		}
